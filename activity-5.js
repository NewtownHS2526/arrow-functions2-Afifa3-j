...
const createMultiplier = (factor)=> function (number) {
    return number * factor;
}
...
// Convert to arrow function (convert both outer and inner functions):
const processArray = (arr, callback) => arr . map (function(item) {
    return callback(item);
})
...
// Convert to arrow function (convert both outer and inner functions):
const sortByProperty = (objects, property) => objects . sort (function(a,b){
    return a [property] - b [property];
});
...
const filterAndMap = (number) => {
    return numebr.filter (num => num >0)
    .map (num => num *2);
}
...
// Convert to arrow function (convert both outer and inner functions):
const createValidator = (min, max) =>function (value) {
    return value >= min && value <= max ;
};
// Problem 7
...
// Convert to arrow function (convert both outer and inner functions):
const processData = (data, transform) => data. reduce (function(acc, item) {
    acc.push(transform(item));
    return acc;
}, []);
